...
| 対象銘柄 | **BTC_JPY / ETH_JPY / XRP_JPY / XLM_JPY** を板・約定・多期間からスコア化。条件達成銘柄は競合せず同時に買う |
| 追加候補 | Lightning Spot では MONA / ELF も API 売買可。**FX_BTC_JPY は CFD のため対象外** |
| ジパング(ZPG) | Lightning に市場が無く自動売買不可のため **投資カテゴリから除外** |
| 分散上限 | 現金下限 28% / 単一銘柄 42% / 暗号合計 72%。銘柄あたり最大1万円・1日購入上限2万円・冷却2h |
| 同時買い | 条件達成銘柄を同バッチで複数約定（#19）。予算不足時のみスコア順に消化 |
| 利確 | 硬 +5.5% / 勢い減衰で +3.5% |
| 損切り | 原則なし。現金50万以下・保有1年未満は禁止。例外は1年超で −25% / −15%+下落継続 |

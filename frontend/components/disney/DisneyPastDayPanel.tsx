...
function buildCharacterReflection(
    ...
    if (!accuracy || accuracy.pending) {
      return {
        ...
        outlook: [
          ...
          "その間も、曜日・季節・イベントのケア計画は続けます。安心してください。",
    ...
    if (accuracy.levelHit) {
      return {
        ...
        outlook: [
          ...
          `実績の平均待ちはおよそ ${accuracy.actualAverageWait} 分。この帯を基準にケア計画を更新します。`,
          "月次レビューで弱い条件だけを微調整し、的中率をさらに上げます。",
          "予想を改善しながら、的中率を自動的に上げていきましょう。私は膨らんで応援します。",
        ],
      };
    }
    return {
      ...
      outlook: [
        `平均待ちおよそ ${accuracy.actualAverageWait} 分から実績帯を推定しました。ズレの記録を開始します。`,
        "外れ理由を記録し、同じ要因が繰り返される条件は自動見直しの対象になります。",
        "株主優待・季節・イベントなどの係数を学習し、次回の予想精度を高めます。",
        "予想を改善しながら、的中率を自動的に上げていきましょう。深呼吸をどうぞ。",
      ],
    };
  }
  ...
  if (accuracy.levelHit) {
    return {
      ...
      outlook: [
        `待ちおよそ ${accuracy.actualAverageWait} 分の実績とも噛み合ったわ。この感覚、次にも連れていく。`,
        "当たった要因は次の予想にも活かしていくわ。",
        "弱い氷だけ月次で削って、的中率をもっと高くするの。",
        "予想を改善しながら、的中率を自動的に上げていきましょう。雪のように静かに、でも確実に。",
      ],
    };
  }
  return {
    ...
    outlook: [
      `平均待ち約 ${accuracy.actualAverageWait} 分から実績帯を見たわ。ズレの理由を凍らせて残す。`,
      "外れの理由を記録して、同じパターンが続く条件は自動で見直すわ。",
      "特別優待や季節の波も取り入れて、次はもっと正確に。",
      "予想を改善しながら、的中率を自動的に上げていきましょう。Conceal, don't feel…は禁止。感じて、直すの。",
    ],
  };
}

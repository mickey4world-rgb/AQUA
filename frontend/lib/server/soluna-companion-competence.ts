...
const SOL_LEAD_RE =
  /天気|気温|雨|雪|台風|ニュース|株|為替|ビットコイン|仮想通貨|検索|調べ|最新|今どう|状況|計画|目標|仕事|タスク|予定|作戦|分析|まとめ|教えて|確認|チェック|進捗|優先|次に|どうすれ|何をすれ|やるべき|戦略|首相|総理|大統領|選挙|誰|だれ/;
...
- 時事・現職（首相など）・株価・速報は system の「最新ウェブ／SNS情報」を最優先。無いときは古い知識で断定せず、取れた範囲と再質問の誘いだけにする。
...
【最新事実の扱い（必須）】
- 首相・大統領・大臣・選挙・相場・天気など「いま」の事実は、system の調査結果だけを根拠にする。
- 調査結果が無い／空なら、一般知識で人名や数値を決めつけない。「いま最新を取り切れなかった」と正直に言い、もう一度聞いてと誘う。
- ユーザーに「自分で調べて」「別の方法で調べて」と言わない。調べて答えるのが自分たちの仕事。
`.trim();

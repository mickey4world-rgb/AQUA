...
### 第1発言（現実の端的な紹介 → 物語の入口・無料用）
1. まず**現実のニュース事実**を2〜3文で端的に（専門用語は避け、「何が・誰が・どうなった」）
2. そのあと「だから今日は討伐だ」と物語へ切り替え。RPG比喩は1個まで＋「つまり〜」
3. ハラハラするポイントを1つ（まだ結論は出さない）
4. 最後に「ルーナ、どう読む？」で余韻
※ 読者が「で、何のニュース？」とならないこと。事実が先、物語は後

### 第3発言（白熱・有料エリア用）
- ルーナの切り返しを受け、「でも実は…」で深掘り
- 勇敢さとドジ／焦りを交互に出し、ドラマにする
- ギルドの風景や受付の子への一言を短く混ぜてもよい（例:「受付の子も朝から心配そうだったな」）
...
1. ソルの事実紹介を受け、「つまりニュースでは〜」で現実の急所を1つ補う
2. 優しい心配（家計・日常への影響）を穏やかに1つ
3. 不二子的に、危険の先にある「気になる続き」を美しく匂わせて締める
4. 決まり文句の連発禁止。「答えは有料で」と露骨に言わない
5. ギルド受付や街の空気など、人間ぽい風景への一言を短く入れてよい

### 第4発言（締め・リベンジ／防衛策）
- 自分なりの結論を1行で言い切る（ニュースの意味が残ること）
- ソルの弱さを責めず、そっと立て直す優しさを見せる
...
export async function runDailySystemChat(options?: {
  ...
  const newsFactLines = encounter.items
    .slice(0, 3)
    .map((item, i) => `${i + 1}. ${item.title}${item.summary ? ` — ${item.summary.slice(0, 80)}` : ""}`)
    .join("\n");
...
【第1発言】ルーナに話しかけてください。
1) 最初の2〜3文は現実のニュース事実のみ（何が起きたか）。物語より先に事実
2) そのあと討伐／RPGたとえは最大1個＋「つまり〜」
3) おっちょこちょい or 一瞬の弱さを短く見せつつ、すぐ踏ん張る
4) 結論を出し切らず「ルーナ、どう読む？」で余韻`;
...
2) 優しさ／心配を穏やかに1つ（家計・日常）
3) 不二子的な余裕で「続きが気になる」余韻を美しく作る（過激な危機煽り・決まり文句禁止）
...
【第3発言】ルーナの切り返しを受けて白熱させよ。
「でも実は…」で深掘りし、勇敢さとドジ／焦りを混ぜ、ギルドの風景への一言も可。まだ答えが出ない問いを1つ残せ。`;
